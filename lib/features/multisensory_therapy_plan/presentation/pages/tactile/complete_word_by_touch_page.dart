...
class CompleteWordByTouch extends StatefulWidget {
  const CompleteWordByTouch({super.key, required String category});

  @override
  _CompleteWordByTouchState createState() => _CompleteWordByTouchState();
}

class _CompleteWordByTouchState extends State<CompleteWordByTouch> {
  ...
  void _fetchQuestions() {
    Provider.of<TherapyProvider>(context, listen: false)
        .fetchQuestions('tactile', 'complete_the_word_by_touch');
  }
  ...
  Future<void> _processAnswer(
    ...
    provider.addAnswer(
      ...
      provider.questions
          .where((q) =>
              q.type == 'tactile' && q.category == 'complete_the_word_by_touch')
    ...
    if (isLastQuestion) {
      ...
      try {
        ...
        await provider.submitAnswers('tactile', 'complete_the_word_by_touch');
    ...
    return Consumer<TherapyProvider>(
      builder: (context, provider, child) {
        ...
        final questions = provider.questions
            .where((q) =>
                q.type == 'tactile' &&
                q.category == 'complete_the_word_by_touch')
            .toList();
                                ...
                                children: [
                                  ...
                                  Text(
                                    currentQuestion.description ??
                                        'Listen to the word and draw the missing letter.',
                                    style: const TextStyle(
                                        color: AppColors.textPrimary,
                                        fontSize: 16),
                                  ),
